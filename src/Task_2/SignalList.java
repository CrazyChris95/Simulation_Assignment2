...
public class SignalList {
	...
	public static void SendSignal(int type, Proc dest, double arrtime, int priority){
	 	Signal dummy, predummy;
	 	Signal newSignal = new Signal();
	 	newSignal.signalType = type;
	 	newSignal.destination = dest;
	 	newSignal.arrivalTime = arrtime;
	 	newSignal.priority = priority;
	 	
	 	
	 	predummy = list;
	 	dummy = list.next;
	 	while (((dummy.arrivalTime < newSignal.arrivalTime) & (dummy != last)) || (newSignal.priority>=dummy.priority && dummy.arrivalTime == newSignal.arrivalTime && dummy != last)){
	 		predummy = dummy;
	 		dummy = dummy.next;
	 	}
